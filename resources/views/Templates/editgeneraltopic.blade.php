...
@include('partials.forumheader')
    <br/> 
          ...
          <form class="form-vertical" role="form" method="post" action="{{route('generaltopic.update',['category' => \App\Category::find($topic->category_id)->name,'topicId' => $topic->id, 'slug' => $topic->slug])}}" enctype="multipart/form-data">
      ...
      @if($topic->forumimage3 !== NULL && Auth::user()->id === $topic->user_id)
          ...
          <input type="hidden" name="generaltopicredirect" value="{{URL::previous()}}">
